...
- [x] T610 Set crate and lockfile version to `1.0.0`; update changelog/release documentation without adding product behavior. Version, lockfile, and changelog are canonical through PR #70; the dedicated publication runbook is implemented in the current final candidate and becomes canonical only through T615.
...
- [x] T613 Add immutable published-release verification for `v1.0.0` covering tag lineage, version, immutable state, release attestation, checksums, binary attestations, and every release asset. Implemented in the current final candidate; canonical effectiveness is gated by T614/T615.
- [ ] T614 Qualify one exact publication-implementation head with `ci`, `skills-compat`, `release`, all v1 and historical v0.1 guards, review/thread/comment reconciliation, mergeability, and canonical-main reconciliation.
...
T603 gates publication implementation. Repository diff policy required staging and verifier surfaces to become separate bounded units; T612 is now canonical. The current verifier/runbook candidate is the only remaining T614/T615 implementation unit. T615 gates tag creation. T620 gates staging. T621 gates the administrative publication prerequisite. T632 gates terminal closeout.
