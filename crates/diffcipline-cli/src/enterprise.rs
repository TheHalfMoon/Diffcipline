...
mod tests {
    ...
    fn layering_is_monotonic_and_cumulative() {
        ...
        let policy = merge_policies(&enterprise, &repository);
        assert_eq!(policy.max_changed_files, 4);
        assert_eq!(policy.max_added_lines, 100);
        assert_eq!(policy.dependency_manifest_changes, Decision::Fail);
        assert_eq!(policy.commands, ["enterprise", "repository"]);
        assert_eq!(policy.expected_files, ["src/**", "*.rs"]);
    }

    #[test]
    fn expected_contracts_are_independent() {
