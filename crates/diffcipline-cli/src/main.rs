...
mod core {
    ...
    pub fn main_enterprise() -> ExitCode {
        match run_enterprise() {
    ...
    fn run_enterprise() -> Result<Verdict, String> {
        let args = env::args().skip(1).collect::<Vec<_>>();
        if args.first().map(String::as_str) != Some("check")
            || !args.iter().any(|arg| arg == "--enterprise-policy")
        {
            return run();
        }
        run_check_enterprise(args[1..].to_vec())
    }

    fn run_check_enterprise(args: Vec<String>) -> Result<Verdict, String> {
        let mut base = None;
        let mut risk = None;
        ...
        let enterprise_policy = enterprise_policy.expect("enterprise flag selected this path");
        let (result, sources) = check_enterprise(
            base.as_deref(),
            execute,
            risk,
            enterprise_policy.as_str(),
        )?;
        if json {
            println!(
                "{}",
    ...
    fn read_policy(path: &Path, source: &str) -> Result<Policy, String> {
        ...
        if policy.version != 1 {
            return Err(format!("unsupported policy version in {source}: {}", policy.version));
        }
        Ok(policy)
    }
        ...
        for path in paths {
            for (label, patterns) in [
                ...
                if !patterns.is_empty()
                    && !patterns.iter().any(|pattern| path_pattern_matches(pattern, path))
                {
                    reasons.push(format!("{label} expected-files contract rejected: {path}"));
                }
            }
            if forbidden.iter().any(|pattern| path_pattern_matches(pattern, path)) {
    ...
    mod enterprise_tests {
        ...
        #[test]
        fn layering_is_monotonic_and_cumulative() {
            let enterprise = Policy {
                max_changed_files: 4,
                max_added_lines: 200,
                dependency_manifest_changes: Decision::Fail,
                expected_files: vec!["src/**".into()],
                commands: vec!["enterprise".into()],
                ..Policy::default()
            };
            let repository = Policy {
                max_changed_files: 9,
                max_added_lines: 100,
                dependency_manifest_changes: Decision::Allow,
                expected_files: vec!["*.rs".into()],
                commands: vec!["repository".into(), "enterprise".into()],
                ..Policy::default()
            };
            let merged = merge_policies(&enterprise, &repository);
            assert_eq!(merged.max_changed_files, 4);
            assert_eq!(merged.max_added_lines, 100);
            assert_eq!(merged.dependency_manifest_changes, Decision::Fail);
            assert_eq!(merged.commands, ["enterprise", "repository"]);
            assert_eq!(merged.expected_files, ["src/**", "*.rs"]);
        }

        #[test]
        fn expected_contracts_remain_independent() {
            let reasons = layered_scope_violations(
                &["src/**".into()],
                &["*.rs".into()],
                &[],
                &["README.md".into()],
            ...
            assert!(reasons[0].starts_with("enterprise expected-files"));
            assert!(reasons[1].starts_with("repository expected-files"));
        }

        #[test]
        fn enterprise_flag_requires_one_path() {
            let missing = run_check_enterprise(vec!["--enterprise-policy".into()]);
            assert!(missing.unwrap_err().contains("requires a local file path"));
            let duplicate = run_check_enterprise(vec![
                "--enterprise-policy".into(),
                "one.toml".into(),
                "--enterprise-policy".into(),
                "two.toml".into(),
            ]);
            assert!(duplicate.unwrap_err().contains("only once"));
        }
    }
}

fn main() -> std::process::ExitCode {
    core::main_enterprise()
}
